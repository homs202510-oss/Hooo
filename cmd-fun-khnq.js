...
const globalStickerPath = path.join(__dirname, 'db-globall_choke_sticker.json');
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      if (subCommand === 'حفظ' || subCommand === 'save') {
        ...
        await sock.sendMessage(chatId, {
          text: `✅ تم حفظ ملصق الخنق عاماً!\n━━━━━━━━━━━━━━━━━━━━\n📌 استخدم .خنق @منشن للإرسال\n👤 بواسطة: ${displayName}\n━━━━━━━━━━━━━━━━━━━━\n𝘽𝙔┇𝑷𝑯𝑨𝑵𝑻𝑶𝑴 ❤️`,
      ...
      if (!savedSticker) {
        await sock.sendMessage(chatId, {
          text: `❌ لا يوجد ملصق خنق محفوظ.\n📌 قم بحفظ ملصق أولاً: .خنق حفظ (رد على ملصق)`
        }, { quoted: msg });
        await sock.sendMessage(chatId, { react: { text: '❌', key: msg.key } });
        return;
      }
      ...
      await sock.sendMessage(
        chatId,
        {
          text: `@${senderName} خنق @${targetName} 😵`,
          mentions: [sender, who]
        },
        { quoted: msg }
      );
      ...
      console.error('✗ خطأ في أمر خنق:', error);
      await sock.sendMessage(
        msg.key.remoteJid,
        {
