// تك.js - تحميل صوت من تيك توك (يدعم الروابط والبحث)
...
async function sendMessage(sock, chatId, lines, quoted = null, mentions = []) {
    ...
    let msg = `🎧 تـحـمـيـل صـوت تـيـك تـوك 🎧\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━\n`;
...
module.exports = {
    command: ['تك'],
    category: 'وسائط',
    description: '🎧 تحميل صوت من تيك توك (بحث أو رابط)',
    usage: '.تك [بحث أو رابط]',
    price: commandPrice,
    ...
    async execute(sock, msg, args) {
        try {
            ...
            if (!inputText && msg.message?.conversation) {
                inputText = msg.message.conversation.replace(/^\.تك\s*/i, '').trim();
            }

            if (!inputText && msg.message?.extendedTextMessage?.text) {
                inputText = msg.message.extendedTextMessage.text.replace(/^\.تك\s*/i, '').trim();
            }
            ...
            if (!inputText) {
                const lines = [
                    ...
                    '',
                    '📌 الاستخدام:',
                    '.تك [اسم البحث] - بحث عن صوت',
                    '.تك [رابط] - تحميل من رابط',
                    '',
                    `💰 السعر: ${commandPrice} نقطة`,
                    '',
                    '📝 أمثلة:',
                    '.تك قرآن',
                    '.تك https://vt.tiktok.com/xxxxx'
                ];
            ...
            const title = video.title || video.music_info?.title || 'صوت تيك توك';
